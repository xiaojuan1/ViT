...
class AverageMeter(object):
    ...
    def update(self, val, n=1):
        ...
        self.sum += val * n  # 累加总损失值
        self.count += n  # 更新样本计数
        self.avg = self.sum / self.count  # 计算平均损失值


def accuracy(output, target, topk=(1,)): # topk=(1,) 只考虑最高分对应的预测结果的精度
    """
    ...
    Returns:
        res: 一个列表，包含各个top k值对应的精度百分比
    """
    maxk = max(topk)  # 获取要计算的top k值中最大的一个
    batch_size = target.size(0)  # 获取批次大小，即目标标签的数量
    # 从模型输出中获取前k个最高分的类别预测结果
    _, pred = output.topk(maxk, 1, True, True)
    pred = pred.t()  # 转置，使得每一列对应一个样本的预测结果
    correct = pred.eq(target.view(1, -1).expand_as(pred)) # 将预测结果与目标标签进行比较，得到一个布尔类型的矩阵，表示每个预测是否正确
